...
int mappingf[UPBOUND]={0};
int mappingb[UPBOUND]={0};
double x_[UPBOUND];

static int SMT1=97;//104;//confirmed 185 186

static int TIMES1=70;//75;//confirmed 105   176

int dispatch[NUM_THREADS+1][UPBOUND];
int predispatch[NUM_THREADS+1][UPBOUND];
int *rs_u_;
int *i_u;
double *u_u_;
struct TreeNode
{
	...
	int totalnum;
	int parent;
	int type;//0:leaf    1:normal    2:root   3:split     4:added to dispatch
};
TreeNode node[UPBOUND];
vector<int> rootarray;
int caltotalnum(int x)
{
	if(node[x].type==0)
		node[x].totalnum=1;
	for(int i=0;i<node[x].children.size();i++)
		node[x].totalnum += caltotalnum(node[x].children[i]);
	return node[x].totalnum;
}

void traverse(int x,int y) //x temp root ; y dispatch
{
	dispatch[y][++dispatch[y][0]]=x;
	node[x].type=4;
	for(int i=0;i<node[x].children.size();i++)
		traverse(node[x].children[i],y);

}
void precut(int x,int y,int smt)//cut x , store in predispatch[y][]
{
	predispatch[y][++predispatch[y][0]]=x;
	node[x].type=4;
	for(int i=0;i<node[x].children.size();i++)
	{
	
		int child=node[x].children[i];
		int csc=node[child].totalnum;
		if(csc>smt)
		{
			precut(child,y,smt);
		}
...
void LU_EliminationTreeG(SprsMatRealStru *pG, int* pParent)
{
	...
	istart=pG->Mat.piIstart;// 每行矩阵元素在iJno中的起始位置，维数iDim+2。（IYD1、IYDU）
	linkn=pG->Mat.piLinkn;// 行号纵向链
	linkp=pG->Mat.piLinkp;// 位置纵向链
	...
	for(i=1; i<=iDim; i++)
	{
		...
		jmn = istart[i];// 每行矩阵元素在iJno中的起始位置，维数iDim+2。（IYD1、IYDU）
		kn = linkn[jmn];// 行号纵向链
		kp = linkp[jmn];// 位置纵向链
		if(kp==0) continue;
...
/////////////////////////////////////////////
	...
	for(int i=1;i<=M;i++)
	{
		node[i].totalnum=1;
		node[i].parent=-1;
		node[i].type=1; // normal
		node[i].children.clear();
	}
	for(int i=1;i<M;i++)
	{
		node[pParent[i]].children.push_back(i);
		node[i].parent=pParent[i];
	}
	for(int i=1;i<=M;i++)
	{
		if(node[i].parent==-1) node[i].type=2; //root
		if(node[i].children.size()==0) node[i].type=0; //leaf
	}
	caltotalnum(M);
	rootarray.push_back(M);
	int times=TIMES1;
	while(times--)
	{
		int max=0;
		...
		{
			if(node[rootarray[i]].totalnum>max && node[rootarray[i]].type==2)
			{
				max=node[rootarray[i]].totalnum;
				maxindex=rootarray[i];
			}
		}
		for(int i=0;i<node[maxindex].children.size();i++)
		{	
			int child=node[maxindex].children[i];
			node[maxindex].type=3;
			node[child].type=2;
			rootarray.push_back(child);
		}
	}
	int smt=SMT1;
	for(;;)//cut until all is smaller than ...
	{
		int max=0;
		int maxindex=-1;
		for(int i=0;i<rootarray.size();i++)
		...
		if(max<=smt) break;
		int y;
		int biggestsize=1000000;
		for(int k=1;k<=NUM_THREADS;k++)
		{
			if(predispatch[k][0]==0 ||predispatch[k][0]<biggestsize )
			{
				y=k;
				biggestsize=predispatch[k][0];
				if(predispatch[k][0]==0) break;
			}

		}
		precut(maxindex,y,smt);//cut maxindex , store in predispatch[y][]
	...
	for(;;)
	{
		...
		{
			int x=rootarray[i];
			if(node[x].type==2 && node[x].totalnum>max)
			{
				max=node[x].totalnum;
				selected=x;
			}
		...
		if(selected!=-1)
		{
			...
			for(int k=1;k<=NUM_THREADS;k++)
			{
				if(dispatch[k][0]==0 ||dispatch[k][0]<biggestsize )
				{
					y=k;
					biggestsize=dispatch[k][0];
					if(dispatch[k][0]==0) break;
				}
	...
	for(int i=0;i<=NUM_THREADS;i++)
	{
		sort(&dispatch[i][1],&dispatch[i][1]+dispatch[i][0]); 
		sort(&predispatch[i][1],&predispatch[i][1]+predispatch[i][0]); 
	}


//前推索引号存入mappingf[],回代索引号存入mappingb[]
	int index=1;
	//存第一层子树
	for(int i=1;i<=NUM_THREADS;i++)
	{	
		for(int j=1;j<=dispatch[i][0];j++)
		{
			mappingf[index]= dispatch[i][j];
			mappingb[dispatch[i][j]]=index;
			dispatch[i][j]=index;
			index++;
		}
	}
	...
	for(int i=1;i<=NUM_THREADS;i++)
	{	
		for(int j=1;j<=predispatch[i][0];j++)
		{
			mappingf[index]= predispatch[i][j];
			mappingb[predispatch[i][j]]=index;
			predispatch[i][j]=index;
			index++;
		}
	}
    //存剩余节点
	...
	for(int i=0;i<=NUM_THREADS;i++)
	{
		...
		b+=predispatch[i][0];
		printf("%d thread:",i);
		for(int j=0;j<=dispatch[i][0];j++)
		{
			printf("%d ",dispatch[i][j] );
		}
		printf("\n");
		printf("\n");
	}
	...
	{
		printf("%d thread:",i);
		for(int j=0;j<=predispatch[i][0];j++)
		{
			printf("%d ",predispatch[i][j] );
		}
		printf("\n");
		printf("\n");
	}
...
void LU_SetUMatECountsG(SprsMatRealStru *pG,int* pParent,SprsUMatRealStru *pU)
{
	...
	rs_u_=(int *)calloc(iDim+2,sizeof(int));
	...
	u_u_=(double *)calloc(pU->uMax.iNzs+1,sizeof(double));
	...
	i_u=(int *)calloc(pU->uMax.iNzs+1,sizeof(int));
...
// 其    他:          // 不会影响G阵中矩阵的任何信息,created by xdc 2014/6/16
//////////////////////////////////////////////////////////////////////
//void LU_SymbolicSymG(SprsMatRealStru *pG,SprsUMatRealStru *pFU,int times,int smt)
...
void LU_NumbericSymG(SprsMatRealStru *pG,SprsUMatRealStru *pFU)
{
	...
	int ks,ke;
	int col=200,intvalT=iDim+1;//potential bug
	int **data;
	data = new int*[intvalT];
	double **datau;
	datau = new double*[intvalT]; 
	int  *index=new int [intvalT];
	for(int j=0;j<intvalT;j++)
	{
		data[j] = new int[col];
		datau[j] = new double[col];
	}

	for (int i=0;i<intvalT;i++)
	{
		index[i]=0;
		for (int j=0;j<col;j++)
		{
	...
	for( i=1;i<intvalT;i++)
	{
		for( j=0;j<index[i];j++)
		{
			u_u_[tempindex]=datau[i][j];
			i_u[tempindex]=data[i][j];
			tempindex++;
		}
	}
	...
	int *temp_rs_u_=(int *)calloc(iDim+2,sizeof(int));
	int *temp_i_u=(int *)calloc(pFU->uMax.iNzs+1,sizeof(int));
	double *temp_u_u_=(double *)calloc(pFU->uMax.iNzs+1,sizeof(double));

	temp_rs_u_[1]=1;
	int ti=1;
	for(int i = 1; i <= iDim; i++)
	{
		int reali=mappingf[i];
		for(int k=rs_u_[reali];k<rs_u_[reali+1];k++)
		{
			temp_i_u[ti]=mappingb[i_u[k]];
			temp_u_u_[ti]=u_u_[k];
			ti++;
		}
		temp_rs_u_[i+1]=ti;
	...
	for(int i=0;i<iDim+2;i++)
		rs_u_[i]=temp_rs_u_[i];
	for(int i=0;i<pFU->uMax.iNzs+1;i++)
		i_u[i]=temp_i_u[i];
	for(int i=0;i<pFU->uMax.iNzs+1;i++)
		u_u_[i]=temp_u_u_[i];
	...
	int *temp_rs_u=(int *)calloc(iDim+2,sizeof(int));
	int *temp_j_u=(int *)calloc(pFU->uMax.iNzs+1,sizeof(int));
	double *temp_u_u=(double *)calloc(pFU->uMax.iNzs+1,sizeof(double));
	...
	{
		int reali=mappingf[i];
		for(int k=rs_u[reali];k<rs_u[reali+1];k++)
		{
			temp_j_u[ti]=mappingb[j_u[k]];
			temp_u_u[ti]=u_u[k];
			ti++;
		}
		temp_rs_u[i+1]=ti;
	...
	double *temp_d_u=(double *)calloc(iDim+1,sizeof(double));
	for(int i=0;i<iDim+1;i++)
		temp_d_u[i]=d_u[mappingf[i]];
	for(int i=0;i<iDim+1;i++)
		d_u[i]=1/temp_d_u[i];
	free(temp_d_u);
