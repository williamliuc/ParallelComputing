...
void LU_EliminationTreeG(SprsMatRealStru *pG, int* pParent)
{
   ...
   linkp=pG->Mat.piLinkp;// 位置纵向链, 矩阵中同一列下一个元素的位置，矩阵最后一行元素的linkp指向第一个元素,维数iNymax+1（LP1、LPU）
   linkn=pG->Mat.piLinkn;// 行号纵向链，下个元素的行号，矩阵最后一行元素的linkn为第一元素行号,维数iNymax+1。（LR1、LRU）
   istart=pG->Mat.piIstart;// 每行矩阵元素在iJno中的起始位置，维数iDim+2。（IYD1、IYDU）
	...
	for(i=1; i<=iDim; i++)
	{
      ...
      jmn = istart[i];//元素实际存储位置
      kn = linkn[jmn];//元素在矩阵中所在行号
      kp = linkp[jmn];//元素在矩阵中所在列号
      if(kp==0) continue;
